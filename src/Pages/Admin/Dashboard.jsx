...
import { FaUsers, FaDollarSign, FaHome } from "react-icons/fa";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = () => {
      ...
      const totalRevenue = bookings.reduce(
        (sum, booking) => sum + (booking.status === "Paid" ? booking.total : 0),
        0
      );
      ...
      setStats({
        ...
        occupancy_rate: totalBookings > 0 ? 75 : 0,
      });
  ...
  const availableRoomsCount = rooms.filter((room) => room.status === "available")
    .length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
          <div className="flex items-center justify-between">
            ...
            <div className="bg-teal-50 p-3 rounded-full text-blue-500 font-bold text-3xl">
              <FaDollarSign />
            </div>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {localBookings.map((booking, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">{booking.address}</td>
